...
*/
    ...
    /*
    If we got this far, then we need to interpolate because the sampling is
    not fine enough and/or because the reference pressure is not an input
    pressure. The latter case has two sub-cases: the reference pressure is
    either between input pressures, or it is shallower than p_min, requiring
    extrapolation.  Bad idea!
    */
    ...
    if ((dp_max <= max_dp_i) && (ipref >= 0)) {
        ...
        ! there is a "bottle" at exactly p_ref, but
        */
